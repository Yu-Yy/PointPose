...
class PatchTransformerEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        embeddings = embeddings + self.positional_encodings[:embeddings.shape[2], :].T.unsqueeze(0) # combine way is summation

        # change to S,N,E format required by transformer
        embeddings = embeddings.permute(2, 0, 1) # E is fixed as 128,    S ,batch, E 
...
class PixelWiseDotProduct(nn.Module):
    ...
    def forward(self, x, K):
        ...
        y = torch.matmul(x.view(n, c, h * w).permute(0, 2, 1), K.permute(0, 2, 1))  # .shape = n, hw, cout # 1x1 卷积等同于矩阵乘法
