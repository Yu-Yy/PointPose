...
class mViT(nn.Module):
    ...
    def forward(self, x):
        ...
        regression_head, queries = tgt[0, ...], tgt[1:self.n_query_channels + 1, ...]  # query_channel < sequence num
        ...
        if self.norm == 'linear':
            ...
            y = torch.sigmoid(y)
        y = y / y.sum(dim=1, keepdim=True) # keepdim 
        return y, range_attention_maps # y is the bin's norm width
