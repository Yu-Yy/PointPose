...
from models.votepose_utils import VotePoseBackbone, VotePoseVoting, VotePoseProposal, get_loss, VotePoseVoting_, VotePoseProposal_, get_loss_, VotePoseBackbone_
...
class VotePoseNet_(nn.Module):
    ...
    def __init__(self, input_feature_dim=0, num_proposal=128, vote_factor=1, sampling='vote_fps'):
        ...
        # Backbone point feature learning
        self.backbone_net = VotePoseBackbone_(input_feature_dim=self.input_feature_dim)

        # Hough voting
        self.vgen = VotePoseVoting_(self.vote_factor, 256) # 128

        # Vote aggregation and detection
        self.pnet = VotePoseProposal_(num_proposal, sampling, self.vote_factor, seed_feat_dim=256) # 128 dim output
    ...
    def forward(self, inputs):
        ...
        end_points = {}
        batch_size = inputs.shape[0]
        end_points = self.backbone_net(inputs, end_points) # changed into float tensor
        ...
        xyz, features = self.vgen(xyz, features)  
        features_norm = torch.norm(features, p=2, dim=1)
        features = features.div(features_norm.unsqueeze(1)) # 归一化？
        end_points['vote_xyz'] = xyz # one suprervise 
        end_points['vote_features'] = features
